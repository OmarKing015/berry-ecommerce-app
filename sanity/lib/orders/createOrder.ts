  ...
  try {
    ...
    if (order) {
      ...
      const stockUpdateItems = orderData.items.map((item) => {
        console.log(
          `Preparing item for stock update: Product _ref (expected productId) = ${item.product._ref}, Quantity = ${item.quantity}, Size = ${item.size}`
        );
        return {
      ...
      console.log(
        "Calling updateMultipleProductsStock with items:",
        JSON.stringify(stockUpdateItems, null, 2)
      );
