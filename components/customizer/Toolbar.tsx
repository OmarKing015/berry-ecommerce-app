...
export default function Toolbar() {
  ...
  const updateShirtColor = useCallback(
    (color: string) => {
  ...
  const handleColorSwatchSelect = useCallback(
  ...
  const generateElementImages = useCallback(async (): Promise<{ name: string; blob: Blob }[]> => {
    if (!canvas) return []
  ...
  const generateFullDesign = useCallback(async (imageUrl: string, canvas: fabric.Canvas): Promise<Blob> => {
    ...
    <TooltipProvider>
      {isMobile ? (
        ...
        >
            ...
            <fieldset disabled={!shirtImageUrl} className="space-y-6 flex-shrink-0">
              ...
              >
                ...
                {!shirtImageUrl && (
                  ...
                  >
                    <p className="text-sm text-muted-foreground text-center">Select a color above to start designing</p>
                  </motion.div>
                )}
          ...
          <div className="flex-shrink-0 p-6 pt-4 border-t border-border/50 bg-gradient-to-t from-background/95 to-transparent backdrop-blur-sm">
            <SignedIn>
              <Button onClick={handleAddToBasket} size="lg" disabled={isLoading || !selectedSize} className="w-full">
                {isLoading ? (
                  <div className="flex items-center">
                    <Loader2 className="animate-spin h-4 w-4 mr-2" />
                    Creating Masterpiece...
